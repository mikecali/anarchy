...
Human application operators may have experience with builds, but often do not have cluster administrative rights required to deploy operators.
...
. Process the `install-template.yaml` to create anarchy custom resource definitions and related cluster Roles:
+
----
...
+
The template requires the parameter `OPERATOR_HOSTNAME`.
This should be a hostname within the domain served by the ingress router.
We recommend using the namespace name as the host name within this domain.
For OpenShift 4, the router ingress domain can be retrived from the `ingresses.config.openshift.io` named `cluster`:
+
----
...
. Set current project namespace to Anarchy operator namespace:
+
----
oc project anarchy-operator
----

. Create a Build Config using the provided template:
+
----
oc process -f build-template.yaml --local | oc apply -n anarchy-operator -f -
----

. Build the anarchy image:
+
----
...
+
----
oc start-build -wF anarchy-runner --from-dir=.
----
...
----
INGRESS_DOMAIN=$(oc get ingresses.config.openshift.io cluster -o jsonpath='{.spec.domain}')
OPERATOR_IMAGE=$(oc get imagestream anarchy -o jsonpath='{.status.tags[?(@.tag=="latest")].items[0].dockerImageReference}')
RUNNER_IMAGE=$(oc get imagestream anarchy-runner -o jsonpath='{.status.tags[?(@.tag=="latest")].items[0].dockerImageReference}')
oc process \
  -f deploy-template.yaml \
  -p OPERATOR_HOSTNAME=anarchy-operator.$INGRESS_DOMAIN \
  -p OPERATOR_IMAGE=$OPERATOR_IMAGE \
  -p OPERATOR_IMAGE_PULL_POLICY=Always \
  -p RUNNER_IMAGE=$RUNNER_IMAGE \
  -p RUNNER_IMAGE_PULL_POLICY=Always \
| oc apply -n anarchy-operator -f -
----
...
+
----
oc new-project anarchy-operator-test
----

. Process and apply test server template
+
----
oc process -f test/server/deploy-template.yaml \
  -p ANARCHY_NAMESPACE=anarchy-operator \
  -p ANARCHY_TEST_NAMESPACE=anarchy-operator-test \
| oc apply -f -
----
...
+
----
ANSIBLE_CONFIG=test/ansible.cfg ansible-playbook test/test-playbook.yaml
----
